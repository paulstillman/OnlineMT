...
special.code.dir = "/Users/jana-mariahohnsbehn/Documents/Online MT try"
...
source("specialized_helper_without_wts.R")

# read in data prepped by data_prep.R
setwd(data.dir)
# long data
l = read.csv("long_data_prepped.csv")
nrow(l) # should be n * number of faces --> gives out 360 (18 x20) 
# wide data
d = read.csv("wide_data_prepped.csv")
nrow(d)  # should be number of subjects --> gives out 20
...
# code ambivalent and non-ambivalent stimuli--> rename confusing into ambivalent
l$confusing = 0
# final 9 foods were ambivalent (confusing: 0 = healthy/univalent; 1= unhealthy/ambivalent)
l$confusing[ l$stim.name %in% paste( "food.", 10:18, sep="" ) ] = 1
...
# average sampling frequency of coordinates in our test sample 20.09
...
summary(lats)
# average latency in our test sample 470.8
...
alerts = describe_alerts( dat = d,
                          ...
                          key = key )
# Many Warnings here...
##---These alerts table command don't function---## 
...
# How should we report the window summaries?? Look at their paper
...
# recode sex ---> remane the variabel in Qualtrics (into gender) and add more options (other, rather not say)
#d$Female = rep( NA, nrow(d) )
d$Female = NA
d$Female[ d$sex == 1] = 1 # Female
d$Female[ d$sex == 2] = 2 # Male
d$Female[ d$sex == 3] = 3 # Diverse
...
#write.csv(table1, "table1.csv") --> this will not coerce, we need to find out why


####################### FIT GEE MODELS #######################
# At first I was unable to fit the Gee models, but then I removes the wts variable because there were only FALSE.
# I also substituted "id=ResponseId" with "id=id"
# After this, I was able to run the models
...
# max x-deviation
library(geepack)
m.xdev = t.test( xdev ~ confusing,
            data = l, paired=TRUE
           )
m.xdev

m.area = t.test( area ~ confusing, 
                 data = l, paired=T)
m.area


# THE PAIRED T TEST WORKS but check if it is really right to use it with long data format

###-------------------------------------------------------#####
## The GLm models I cannot get to work
m.xdev = geeglm( xdev ~ confusing * weird.scaling,
                 data = l,
                 id = id,
                 corstr = "exchangeable" 
)
...
# area
m.area = geeglm( area ~ confusing* weird.scaling , 
            data = l, 
            id = id,
            corstr = "exchangeable" )
summary(m.area)
...
# reaction time
m.rxnt = geeglm( rxnt ~ confusing* weird.scaling, 
            data = l, 
            id = id,
            corstr = "exchangeable" )
summary(m.rxnt)
...
# speed
m.speed = geeglm( speed ~ confusing* weird.scaling, 
            data = l, 
            id = id,
            corstr = "exchangeable" )
summary(m.speed)
...
# x-flips
m.xflips = geeglm( xflips ~ confusing* weird.scaling, 
            data = l, 
            id = id,
            family = poisson,
            corstr = "exchangeable" )
...
for ( i in 1:length(outcomes) ) {
  ...
  m = geeglm( l2[[ outcomes[i] ]] ~ confusing * weird.scaling  + confusing*browser + confusing*OS, 
                data = l2, 
                id = id,
                family = family,
                corstr = "exchangeable" )
...
# I only got this bit to work when I changed the original "my_violins" function definition:
# There, I removed the wts variable and substituted ResponseId with id
...
# This I have no idea how to run without the wts variable
temp = l[ l$weird.scaling == 0, ] # I chnaged this expression
length(unique(temp$id))  # n = 103
